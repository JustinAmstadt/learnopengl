...
Shader::Shader(const char* vertexFile, const char* fragFile, const char* TEFile, const char* TCFile)
{
	if(vertexFile == nullptr){
		std::cerr << "Vertex shader file in shader cannot be empty\n";
		return;
	}
	if(fragFile == nullptr){
		std::cerr << "Fragment shader file in shader cannot be empty\n";
		return;
	}

	this->vertexShaderName = std::string(vertexFile);
	this->fragShaderName = std::string(fragFile);

	if(TEFile != nullptr){
		this->TEShaderName = std::string(TEFile);
	}
	if(TCFile != nullptr){
		this->TEShaderName = std::string(TCFile);
	}
